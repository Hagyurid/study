...
async function renderLibrary(){const packs=await all('packs');await fetchServerPackList();populateLibraryFilters(packs);renderServerPackList();await renderPackList();if(navigator.storage?.estimate){const e=await navigator.storage.estimate();el.storageInfo.textContent=`기기 예상 사용량 ${Math.round((e.usage||0)/1048576)}MB / ${Math.round((e.quota||0)/1048576)}MB`}}
async function renderPackList(){const rows=await all('packs');const packs=applyPackFilters(rows.map(x=>x.pack));buildGroupedPackList(el.packList,packs,{emptyText:'기기에 저장된 문제팩 없음',card:p=>{const d=document.createElement('div');d.className='pack-card local-pack';d.innerHTML=`<b>${esc(p.title)}</b><div class="sub">기기 저장 · ${esc(packSubject(p))} · ${esc(packUnit(p))} · ${packCount(p)}문항</div><div class="row" style="margin-top:8px"><button class="small open">열기</button><button class="small danger delete">삭제</button></div>`;d.querySelector('.open').onclick=()=>loadPack(p.packId);d.querySelector('.delete').onclick=async()=>{if(confirm('기기 저장 문제팩을 삭제할까요? 오답/필기 기록은 별도 progress/strokes에 남을 수 있습니다.')){await del('packs',p.packId);if(state.packSource==='local'&&state.pack?.packId===p.packId){state.pack=null;state.idx=0;await renderAll()}await renderLibrary()}};return d}})}
...
function bindEvents(){/* legacy-null-safe-marker: if($('quickImport'))$('quickImport').onclick */
  ...
  $('openLibrary').onclick=async()=>{await renderLibrary();openModal(el.libraryModal)};
  if($('quickLibrary'))$('quickLibrary').onclick=async()=>{await renderLibrary();openModal(el.libraryModal)};
