# v23 storage-role refactor patch

- Server storage now focuses on original sources, Study Notes, exam cram sheets, unit maps, and main repository problem packs.
- SolvePad separates main repository problem packs, device-stored packs, and wrong-answer management. Server packs open without automatic device-copy.
- CASIO generation defaults to device/local storage (`storage_mode=device`); server archival is opt-in with `storage_mode=server`.
- Added maintenance cleanup for deleted/missing source references, workflow/checkpoint cleanup, WAL checkpoint, and manual VACUUM.
- Dashboard/search behavior is kept compact for lower Render memory pressure.
